...
struct StaredDriverStatus: Widget {
    let kind: String = "StaredDriverStatus"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: WidgetTimeLineProvider()) { entry in
            if #available(iOS 17.0, *) {
                staredDriverStatusEntryView(entry: entry)
        ...
        .configurationDisplayName("Stared driver status")
        .description("Actuel status of stared driver.")
        .supportedFamilies([
                    .systemSmall,
