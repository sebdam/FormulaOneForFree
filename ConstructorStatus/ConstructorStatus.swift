...
struct ConstructorStatus: Widget {
    let kind: String = "ConstructorStatus"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: WidgetTimeLineProvider()) { entry in
            if #available(iOS 17.0, *) {
                constructorStatusEntryView(entry: entry)
        ...
        .configurationDisplayName("Constructor status")
        .description("Actual first constructor status in championship.")
        .supportedFamilies([
                    .systemSmall,
